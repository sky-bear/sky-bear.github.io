...
Node.js 的内存限制是根据系统的物理内存大小动态分配的，并且在不同操作系统下可能会有所不同。在大多数情况下，Node.js 默认的内存限制应该是 1.4 GB（1024 _ 1024 _ 1400 字节）,（64 位系统下约为 1.4 GB，32 位系统下约为 0.7 GB）。但是请注意，这个限制在不同的版本和设置中可能会有所不同
