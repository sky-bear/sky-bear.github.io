...
```js
<link rel="prefetch" href="//www.baidu.com">
<link rel="preload" href="//www.baidu.com">
```
...
提前加载一个非常小的文件， 缓存 DNS,方便后期同域名的资源加载
...
> TCP（传输控制协议）是一种面向连接的、可靠的、基于字节流的传输层通信协议。它提供了一种可靠的、有序的、无重复的数据传输方式，保证数据传输的完整性和可靠性。

#### TCP 连接的建立

TCP 连接的建立需要经过三次握手的过程，三次握手的过程如下：
...
> `SYN`: synchronize 用于建立连接时同步序列号 【1 表示请求连接报文 0 表示这不是一个连接请求报文。在正常的数据传输过程中使用，用于确认已经接收到的数据】
...
- 此时，ACK 标志位为 0，确认号 ack 为 0，序列号 seq 为随机值 x
- 状态变为 SYN_SENT

2. 第二步

- 服务端收到 SYN 后，回应一个 SYN=1 和 ACK=1（确认标志位为 1）
- 确认号 ack 为 x+1，表示服务器期望收到的下一个报文段的第一个数据字节序号是 x+1，同时服务器生成自己的初始序列号 seq 为 y
- 状态变为 SYN_RCVD

3. 第三步

- 客户端发送确认报文，SYN 为 0 ，ACK 标志位设置为 1，确认号 ack 为 y+1，序列号 seq 为 x+1。
- 双方连接建立成功，可以开始通信
...
参看 node 里面的 http 差异
优化： 1.0 1.1 2.0
1.1 - 长连接 | keep-alive，复用 TCP 连接，减少 TCP 连接的建立和关闭次数
2.0 - 多路复用 ，复用 TCP 连接，同时传输多个请求和响应，减少 TCP 连接的建立和关闭次数, 采用二进制帧格式，头部信息更小，传输效率更高，
3.0 - QUIC 协议，基于 UDP，支持多路复用，减少 TCP 连接的建立和关闭次数，支持快速重传，减少丢包重传次数，支持快速恢复，减少连接中断次数
:::

:::warning
http 和 https 的区别 安全性

https 多次连接问题=》 导致网络请求加载时间延长
优化： 合并请求 + 长连接
:::
